...
function selectSkillBuild( selectBox, pokemonSkillInfo ) {
    ...
    selectSkillList[ 0 ].querySelectorAll( 'img' )
        .forEach( ( img ) => {
    ...
    selectSkillList[ 1 ].querySelectorAll( 'img' )
        .forEach( ( img ) => {
            img.addEventListener( "click", ( event ) => {
                const parentNode = event.currentTarget.parentNode.querySelectorAll( 'img' );
    ...
    selectSkillList[ 2 ].querySelectorAll( 'img' )
        .forEach( ( img ) => {
            img.addEventListener( "click", ( event ) => {
                const parentNode = event.currentTarget.parentNode.querySelectorAll( 'img' );
        ...
        try {
            for ( let i = 0; i < 4; ++i ) {
                pokemonDetailInfo[ 'skill' ].forEach( ( skill ) => {
                    if ( skill[ 'name' ] === selected[ i ].getAttribute( 'id' )
                        .substring( 9 ) ) {
            ...
            for ( let i = 4; i < 7; ++i ) {
                heldItemsInfo.forEach( ( heldItem ) => {
                    if ( heldItem[ 'name' ] === selected[ i ].getAttribute( 'id' )
                        .substring( 9 ) ) {
            ...
            battleItemsInfo.forEach( ( battleItem ) => {
                if ( battleItem[ 'name' ] === selected[ 7 ].getAttribute( 'id' )
                    .substring( 9 ) ) {
            ...
            // test
            postBody[ 'position' ] = document.querySelector( '.area.active' )
                .getAttribute( 'id' );
